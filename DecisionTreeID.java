...
public class DecisionTreeID {
    ...
    public static void learnDT(String ficheroCSV) {
        ...
        for (int i = 1; i < features.length; i++) {
            System.out.print("Entropy of " + features[i] + ":");
            System.out.println(calculateEntropyOfNode(data, i, targetIndex));
        }
    ...
    private static double calculateEntropyOfNode(List<String[]> data, int featureIndex, int targetIndex) {
        ...
        for (String possibleValue: rowsOfValue.keySet()) {
            List<String[]> rowsOfPossibleValue = rowsOfValue.get(possibleValue);
            accumulation += (rowsOfPossibleValue.size() / (0.0f + data.size())) * calculateEntropyOfBranch(rowsOfPossibleValue, targetIndex);
        }
    ...
    /* private static ArrayList<String[]> RemoveLine(ArrayList<String[]> data,int node_idx, String value_node) {
        ArrayList<String[]> new_list = new ArrayList<>();
        for(int i = 0;i<data.size(); i++) {
            if(data.get(i)[node_idx] == value_node) {
  ...
  private static ArrayList<String[]> RemoveColumn(ArrayList<String[]> data,int node_idx){
    ArrayList<String[]> new_list = new ArrayList<>();
    for(int i = 0;i<data.size(); i++) {
     for(int j = 0; j<data.get(i).length; i++) {
      if(node_idx != j) {
       new_list.add(data.get(i));
      }
     }
  ...
  private static ArrayList<String[]> GetNewTable(ArrayList<String[]> data,int node_idx, String value_node){
    ArrayList<String[]> new_list = RemoveLine(data, node_idx, value_node);
